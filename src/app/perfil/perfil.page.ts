...
export class PerfilPage {
  ...
  constructor(private firestoreService: FirestoreService, private router: Router, public platform: Platform,
  ...
  loadUserData() {
    ...
    this.firestoreService.getAdmin(firebase.auth().currentUser.uid).pipe(take(1)).subscribe((userSnapshot) => {
  ...
  goRegistro(){
    this.vibracion();
    this.router.navigate(['/registro-datos'], {queryParams: {adminID:firebase.auth().currentUser.uid}});
  }
