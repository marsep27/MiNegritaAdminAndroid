  ...
  loginFacebook() {
    ...
    if (this.platform.is("cordova")) { //Si la plaforma es coordova muestra la siguiente información
      this.auth.loginFacebook().then((res) => {
        this.presentLoading();
        var docRef = firebase.firestore().collection('administradores').doc(res.user.uid);
      ...
      /*this.fb.login(['public_profile', 'email']).then((res: FacebookLoginResponse) => {
        ...
        var docRef = firebase.firestore().collection('administradores').doc(res.authResponse.userID);
        docRef.onSnapshot((async(doc) => {
          if (doc.exists){
            this.router.navigate(['/perfil']);
          }else{
      ...
      firebase.auth().signInWithPopup(provider).then(result => {
        this.presentLoading();
        var docRef = firebase.firestore().collection('administradores').doc(result.user.uid);
        docRef.onSnapshot((async (doc) => {
  ...
  loginGoogle() {
    ...
    if (this.platform.is("cordova")) {//Si la plaforma es coordova muestra la siguiente información
      ...
      /*this.gp.login({}).then(res => {
        ...
        var docRef = firebase.firestore().collection('administradores').doc(res.userId);
        alert(res.userId);
        docRef.onSnapshot((async (doc) => {
          if (doc.exists) {
            this.router.navigate(['/perfil']);
