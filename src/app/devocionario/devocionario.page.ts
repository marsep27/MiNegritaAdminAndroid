...
export class DevocionarioPage {
  ...
  ngOnInit() {
    ...
    //Se obtienen las lecturas de la base de datos
  ...
  ionViewWillEnter() {
    ...
    this.hideOptionsModalLectura();
    this.hideOptionsModalMeditacion();
    this.loadFavourites();
    this.ngOnInit();
  }
  ...
  //Muestra la lectura
  showLectura(title: string, subtitle: string, text: string, orden: number) {
    this.hideOptionsModalLectura();
    this.currTitle = title;
    this.currSubtitle = subtitle;
    this.currText = text;
    this.currText = this.currText.split("\\n\\n").join("\n\n");
    this.currOrden = orden;
  ...
  showNextReading() {
    ...
    if ((index + 1) == this.lecturas.length) {
      this.showLectura(this.lecturas[0].title, this.lecturas[0].subtitle, this.lecturas[0].text, this.lecturas[0].orden);
    }
    else {
      var nextIndex = index + 1;
      this.showLectura(this.lecturas[nextIndex].title, this.lecturas[nextIndex].subtitle, this.lecturas[nextIndex].text, this.lecturas[nextIndex].orden);
    }
  }

  //Muestra la meditación
  showMeditation(title: string, text: string, orden:number) {
    this.hideOptionsModalMeditacion();
    this.currTitle = title;
    this.currText = text;
    this.currText = this.currText.split("\\n\\n").join("\n\n");
  ...
  showNextMeditation() {
    ...
    if ((index + 1) == this.meditaciones.length) {
      this.showMeditation(this.meditaciones[0].title, this.meditaciones[0].text, this.meditaciones[0].orden);
    }
    else {
      var nextIndex = index + 1;
      this.showMeditation(this.meditaciones[nextIndex].title, this.meditaciones[nextIndex].text, this.meditaciones[nextIndex].orden);
    }
  }
  ...
  //Se muestra un popUp con la información 
  showOptionsModalLectura() {
    this.vibracion();
    document.getElementById("dots-modal-lec").style.display = "block";
    document.getElementById("header").style.filter = "blur(2px)";
    document.getElementById("segment").style.filter = "blur(2px)";
    document.getElementById("foot").style.filter = "blur(2px)";
  }
  ...
  //Se muestra un popUp con la información 
  showOptionsModalMeditacion() {
    this.vibracion();
    document.getElementById("dots-modal-med").style.display = "block";
    document.getElementById("header").style.filter = "blur(2px)";
    document.getElementById("segment").style.filter = "blur(2px)";
    document.getElementById("foot").style.filter = "blur(2px)";
  }
  ...
  showReadingsOptionsModal(title: string, subtitle: string, text: string, optId: string, orden:number) {
    this.currTitle = title;
    this.currSubtitle = subtitle;
    this.currText = text;
  ...
  showMeditationsOptionsModal(title: string, text: string, optId: string, orden:number) {
    this.currTitle = title;
    this.currSubtitle = "";
    this.currText = text;
    ...
    this.currOptionId = optId;
    this.currOrden = orden;
    this.showOptionsModalMeditacion();
  }
  ...
  //Se oculta el popUp 
  hideOptionsModalLectura() {
    document.getElementById("dots-modal-lec").style.display = "none";
    document.getElementById("header").style.filter = "none";
    document.getElementById("segment").style.filter = "none";
    document.getElementById("foot").style.filter = "none";
  }
  ...
  //Se oculta el popUp 
  hideOptionsModalMeditacion() {
    document.getElementById("dots-modal-med").style.display = "none";
    document.getElementById("header").style.filter = "none";
    document.getElementById("segment").style.filter = "none";
    document.getElementById("foot").style.filter = "none";
  }
  ...
  see() {
    ...
    switch (this.currOption) {
      case 'prayer':
        ...
        break;
      case 'reading':
        this.showLectura(this.currTitle, this.currSubtitle, this.currText, this.currOrden);
        break;
      case 'song':
        this.openPlayer(this.currTitle, this.currSubtitle, this.currAudio, this.currOptionId);
        break;
      case 'meditation':
        this.showMeditation(this.currTitle, this.currText, this.currOrden);
        break;
    }
  }
  ...
  //Se redirige a la sección de cambiar la lectura
  modificarLectura(title: string, subtitle: string, text: string, optId: string, orden: number) {
    this.router.navigate(['/modificar-lectura'],{
      queryParams: { title: title, subtitle: subtitle, text: text, idLec: optId, orden: orden}});
  }

  //Elimina la lectura seleccionada
  eliminarLectura(title: string, optId: string, opt: string) {
    this.hideOptionsModalLectura();
    this.Delete(title, optId, opt)
  }

  //Se redirige a la sección de cambiar la meditación
  modificarMeditacion(title: string, text: string, optId: string, orden: number) {
    this.router.navigate(['/modificar-meditacion'],{
      queryParams: { title: title, text: text, idMed: optId, orden: orden}});
  }

  //Elimina la meditación seleccionada
  eliminarMeditacion(title: string, optId: string, opt: string) {
    this.hideOptionsModalMeditacion();
    this.Delete(title, optId, opt)
  }

  //Alerta para Eliminar lectura o meditación
  async Delete(title, optId, opt) {
    this.vibracion();
    const alert = await this.alertController.create({
      message: '¿Estás seguro de que querés eliminar '+title+'?',
      buttons: [
        {
          text: 'Sí',
          handler: () => {
            this.vibracion();
            this.confirmarDeleteDefnitivo(title, optId, opt)
          }
        },
        {
          text: 'No',
  ...
  //Alerta para confirmar Eliminar la lectura o meditación
  async confirmarDeleteDefnitivo(title, optId, opt) {
    this.vibracion();
    const alert = await this.alertController.create({
      message: 'Estás a punto de eliminar '+title+'.',
      buttons: [
        {
          text: 'Sí',
          handler: () => {
            ...
            switch (opt){
              case 'lectura':
                this.firestoreService.deleteLectura(optId);
                break
              case 'meditacion':
                this.firestoreService.deleteMeditacion(optId);
                break
            }
          }
        },
